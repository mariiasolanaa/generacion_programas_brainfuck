#PROGRAMA PRINCIPAL PARA REALIZAR UNA SUMA DE DOS NÚMEROS
import random
...
def generaciones(lista_casos_prueba: list[tuple[int, int]], tamaño_poblacion: int = 100, 
                 tamaño_programa: int = 30, max_generaciones: int = 1000, prob_cruce: float = 0.4, 
    ...
    #while (num_generacion <= max_generaciones and not sol_encontrada):
        fitnesses = [evaluar_fitness(prog, lista_casos_prueba) for prog in poblacion]
        ...
        demo1, demo2 = lista_casos_prueba[0]
        demo = chr(demo1) + chr(demo2)
        
        try:
            salida_real = ejecutar_bf(mejor_prog, entrada = demo)
            if salida_real:
                sal_txt = str(ord(salida_real[0]))
            else:
                sal_txt = "(vacio)"
        except (BrainfuckSyntaxError, BrainfuckRunError, BrainfuckStepLimitError):
            sal_txt = 'ERROR'
            
        print(f"Generacion: {num_generacion}  |  fitness: {mejor_fitness}  |  salida[{demo1} + {demo2}]: {sal_txt}")
...
#generaciones([(2,3)])
#generaciones([(2,2), (1,3)])
#generaciones([(2,2), (1,4)])
#generaciones([(2,3), (3,5)])
#generaciones([(2,5), (10,20)])
#generaciones([(2,5), (10,20), (0,1)])